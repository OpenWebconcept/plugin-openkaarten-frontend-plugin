 ...
 * @constructor
 */
const OKP_build_blocks = ( folder, outputFolder = folder ) => {
	Array.from( getDirectories( folder ) ).forEach( ( companyDir ) => {
		Array.from( getDirectories( `${ folder }/${ companyDir }` ) ).forEach( ( blockDir ) => {
			Array.from( getDirectories( `${ folder }/${ companyDir }/${ blockDir }/assets/` ) ).forEach( ( typeDir ) => {
				OKP_build_files( typeDir, `${ folder }/${ companyDir }/${ blockDir }/assets/${ typeDir }`, `${ outputFolder }/${ companyDir }/${ blockDir }` );
			} );
		} );
	} );
};
 ...
 * @constructor
 */
const OKP_build_files = ( typeDir, path, outputPath = path ) => {
 ...
 * @constructor
 */
const OKP_build_client = ( folder, outputFolder = folder ) => {
	Array.from( getDirectories( folder ) ).forEach( ( typeDir ) => {
		OKP_build_files( typeDir, `${ folder }/${ typeDir }`,  outputFolder );

		const directories =  Array.from( getDirectories( `${ folder }/${ typeDir }` ) );
		if ( directories.length > 0 ) {
			directories.forEach( ( dir ) => OKP_build_files( typeDir, `${ folder }/${ typeDir }/${ dir }`, `${ outputFolder }/${ dir }` ) );
		}
	} );
};
...
OKP_build_blocks( 'src/blocks', 'blocks' );
OKP_build_client( 'src/client', 'client' );
