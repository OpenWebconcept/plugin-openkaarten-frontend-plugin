...
class I18n {
	...
	public function load_plugin_textdomain() {
		 ...
		 * 1. $locale using get_locale() is the site locale, not the user locale. - The user locale could be different, so this is not correct.
		 *    I noticed this during debugging, load_plugin_textdomain uses determine_locale(), not get_locale().
		 ...
		 * 4. This error message is printed before any legitimate output, breaking every POST, potentially breaking every AJAX/REST call and at the
		 *    minimum showing the error in raw HTML. At least this should be an error_log, but preferably a wrapper into Admin Notices.
		 *    In my case it only broke wp-admin as I have set Admin to English whereas the site is in Dutch, but in case of a non-NL site,
		 *    this would break the site.
		 * 5. Since all source texts are in English; at least, the error should not show on `en_*` locales.
		 * 6. Finally, the filter construction here is interesting; It is only used here, so it doesn't do anything, also, the expected intention is
		 *    to allow the user to change the locale using a filter, but that is not how WordPress works (unfortunately).
		 */
